...
현재 배포본: [0.4.30 베타 다운로드 및 안내](https://github.com/svsivsv/TaskbarMonitor/releases/tag/v0.4.30). 해당 페이지의 `TaskbarMonitor.exe`를 받으면 됩니다.
...
시작 직후에는 작업표시줄 배치가 0.5초 동안 일치하는지 확인한 뒤 표시합니다. 시작 후 5초 및 설정창 열기·닫기 직후에는 위치를 빠르게 재확인하고 이후에는 저빈도 조회로 돌아갑니다. 빈 공간이 줄면 버튼을 가리지 않도록 즉시 반영하며, 늘어난 공간은 안정된 뒤 반영합니다.
...
검사는 실제 표시 영역 계산과 픽셀 경계, 세 표시 모드의 데이터 갱신, 센서값 만료·복구, 작업표시줄 공간 계산, 그래프 저장 개수를 확인합니다. 결과 JSON은 임시 폴더에만 저장하며 사용자 설정이나 실행 중인 위젯을 조작하지 않습니다. GitHub Actions에서도 같은 검사를 실행합니다. 실제 PC에서의 우클릭·설정 드롭다운·절전 복귀·여러 모니터 조작 검사는 별도로 필요합니다.
