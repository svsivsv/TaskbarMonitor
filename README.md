...
21개 회귀 검사는 표시 영역 계산과 픽셀 경계, 세 표시 모드의 데이터 갱신, 센서값 만료·복구, 작업표시줄 공간 계산, 그래프 저장 개수와 시간축, 기본값·잘못된 설정 복구, 숨김 중 측정 선택, 클릭 통과, 마지막 페이지 채우기, 팝업 크기 범위, 설정창 하단 버튼 줄의 고정 구조와 최소 너비를 확인합니다. 실행 권한과 Windows 호환성 선언도 검사합니다. 결과 JSON은 임시 폴더에만 저장하며 사용자 설정이나 실행 중인 위젯을 조작하지 않습니다. GitHub Actions에서는 저장소의 EXE와 소스에서 새로 빌드한 EXE를 각각 검사합니다.
