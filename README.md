...
| Windows 시작 시 자동 실행 | 현재 사용자 로그인 약 20초 후 저장된 설정으로 실행하는 예약 작업을 등록합니다. 실패 시 1분 간격으로 최대 3회 재시도합니다. 끄고 저장하면 등록을 제거합니다. |
...
- 자동 실행은 현재 사용자 전용 Windows 예약 작업을 사용합니다. 관리자 권한·저장된 암호·네트워크 연결은 필요하지 않으며 배터리에서도 실행합니다. Windows 시작 앱 목록의 기존 Run 항목 대신 앱 설정에서 켜고 끕니다. 이전 EXE에서 업데이트했다면 체크 상태를 확인하고 `저장·적용`을 눌러 새 방식으로 등록하세요. 재등록에 성공한 뒤에만 기존 Run 항목을 제거합니다. 탐색기가 재시작되면 트레이 아이콘과 작업표시줄 배치를 다시 확인합니다.
...
23개 회귀 검사는 표시 영역 계산과 픽셀 경계, 세 표시 모드의 데이터 갱신, 센서값 만료·복구, 작업표시줄 공간 계산, 그래프 저장 개수와 시간축, 기본값·잘못된 설정 복구, 숨김 중 측정 선택, 클릭 통과, 마지막 페이지 채우기, 팝업 크기 범위, 설정창 하단 버튼 줄의 고정 구조와 최소 너비, 오류 기록의 개인 경로·메시지 제외, 자동 실행 예약 작업의 지연·재시도·실행 권한 설정을 확인합니다. 배포 폴더의 EXE 단독 구성, Git 로그 파일 제외, 실행 권한과 Windows 호환성 선언도 검사합니다. 결과 JSON은 임시 폴더에만 저장하며 사용자 설정이나 실행 중인 위젯을 조작하지 않습니다. GitHub Actions에서는 저장소의 EXE와 소스에서 새로 빌드한 EXE를 각각 검사합니다.
