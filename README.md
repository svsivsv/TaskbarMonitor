...
Windows 11 작업 표시줄 안에 CPU·GPU 온도, CPU, 메모리, 디스크, 네트워크, GPU 사용량과 미니 그래프를 표시하는 가벼운 네이티브 위젯입니다.
...
위젯은 Windows 날씨 버튼과 시작 버튼의 실제 위치를 읽어 두 버튼 사이의 빈 공간에 맞춰집니다. `작업표시줄 배경 투명`을 사용하면 별도 패널처럼 보이지 않고 글자와 그래프만 표시됩니다.
...
- 작업표시줄 안쪽에서는 모든 항목 칸을 같은 너비로 유지합니다. `공간에 자동 맞춤`을 켜면 항목을 꺼도 날씨와 시작 버튼 사이에서 확보한 전체 폭은 줄이지 않고, 남은 항목이 그 공간을 나눠 사용합니다. 글자 측정 여백만 부족할 때는 원래 비율을 유지하고 실제 공간이 부족할 때만 온도 글자를 가로로 압축합니다.
...
- 설정창이 열려 있는 동안에는 이미 실행 중인 작업표시줄 위젯을 계속 표시하고 레이어도 유지합니다. Windows가 작업표시줄을 다시 그리거나 다른 모니터의 전체화면 상태가 바뀌어도 설정창을 닫을 때까지 위젯이 사라지지 않습니다.
...
설정창은 내용 영역만 스크롤되고, 아래의 `기본 설정 복원`, `닫기`, `적용`, `저장하고 표시 적용` 버튼 줄은 창 하단에 고정되어 항상 보입니다. `기본 설정 복원`은 화면의 모든 값을 처음 설치 상태로 불러오며, 확인 후에도 `적용` 또는 `저장하고 표시 적용`을 눌러야 실제 설정에 반영됩니다.
...
`적용`과 `저장하고 표시 적용`은 모두 설정창을 유지한 채 현재 위젯에 반영합니다. 설정창은 `닫기` 버튼이나 제목 표시줄의 X를 눌렀을 때만 닫힙니다. 실시간 미리보기의 폭은 현재 작업표시줄에서 실제로 사용할 수 있는 폭과 같은 기준으로 계산하므로, 표시 항목을 끄더라도 실제 위젯과 따로 줄어들지 않습니다.
...
- 하드웨어 측정은 UI와 별도 작업에서 수행하므로 느린 네트워크·디스크 카운터가 메뉴와 설정 조작을 멈추게 하지 않습니다.
...
기본 설정에서는 CPU와 GPU 온도가 켜져 있으며 `CPU 57° 25%`, `GPU 51° 11%`처럼 이름과 사용률 사이에 들어갑니다. 설정 화면의 `온도` 열에서 CPU와 GPU를 각각 켜거나 끌 수 있습니다.

- NVIDIA GPU는 설치된 NVIDIA 드라이버의 NVML 센서값을 직접 사용합니다. `nvidia-smi` 프로세스를 반복 실행하지 않으며 별도 DLL도 배포하지 않습니다.
- 노트북 GPU가 절전 상태로 바뀌는 순간 센서 조회가 실패하더라도 마지막 정상 온도를 유지하고, 센서 초기화 실패 시 10초 뒤 다시 연결합니다.
